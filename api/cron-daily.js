...
module.exports = async (req, res) => {
  try {
    ...
    const ramadan = await isRamadanToday(tz);
    ...
    for (const u of users || []) {
      ...
      // Ertalab/kechqurun xabar yuborish
      if (u.notify_daily_morning) {
        await tg("sendMessage", {
          chat_id: u.tg_user_id,
          text:
            `☀️ Bugungi namoz vaqtlari:\n${lines(mapTimes(t1, tz))}` +
            (ramadan && u.notify_ramadan ? "\n\n🌙 Ramazon eslatmalari ON" : "")
        }).catch(() => {});
      }
      ...
      if (u.notify_daily_evening) {
        await tg("sendMessage", {
          chat_id: u.tg_user_id,
          text:
            `🌆 Ertangi namoz vaqtlari:\n${lines(mapTimes(t2, tz))}` +
            (ramadan && u.notify_ramadan ? "\n\n🌙 Ramazon eslatmalari ON" : "")
        }).catch(() => {});
      }

      // Real-time uchun queue
      const inserts = [];
      ...
      if (ramadan && u.notify_ramadan) {
        ...
        inserts.push(
          n(u.tg_user_id, t1.maghrib, "iftar", {
